...
fn convert_i32_to_value_failure(value: i32) -> proto::ValueFailure {
    // Use a match statement to convert the i32 to the corresponding enum variant
    match value {
        0 => proto::ValueFailure::Unspecified,
...
impl From<&proto::Datapoint> for broker::DataValue {
    fn from(datapoint: &proto::Datapoint) -> Self {
        match &datapoint.value_state {
            ...
            Some(Failure(value)) => {
                let val = convert_i32_to_value_failure(*value);
                let result = broker::ValueFailure::from(&val);
                broker::DataValue::ValueFailure(result)
            }
